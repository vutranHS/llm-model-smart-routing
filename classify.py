#!/usr/bin/env python3
"""Standalone port of MiMo Desktop's local query classifier.

Uses the ONNX model + tokenizer shipped with MiMo Desktop.
Predicts:
...
_LOCAL_DIR = Path(__file__).resolve().parent / "query-classifier"
_MIMO_DIR = Path.home() / (
    "Library/Application Support/Xiaomi MiMo AI/on-demand-assets/query-classifier"
)
# Prefer bundled copy next to this script; fall back to MiMo Desktop install.
DEFAULT_DIR = _LOCAL_DIR if (_LOCAL_DIR / "model_fp16.onnx").exists() else _MIMO_DIR

SCENES = ["office", "software", "design", "media", "research", "other"]
DIFFICULTIES = ["easy", "medium", "hard"]

# Heuristic model pick — mirrors Desktop's flash/pro split (displayRatio 0.4 vs 1.0)
def pick_model(scene: str, difficulty: str) -> str:
    if difficulty == "hard":
        return "mimo-x-pro-preview"
    if difficulty == "medium":
        # software/design/research lean pro; others flash
...
def main(argv=None) -> int:
    p = argparse.ArgumentParser(description="Port of MiMo Desktop query classifier")
