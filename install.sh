...
if [[ ! -f "$PREFIX/query-classifier/model_fp16.onnx" ]]; then
  echo "ONNX weights missing — running fetch_classifier.sh"
  "$PREFIX/fetch_classifier.sh" || {
    echo "WARNING: could not fetch model_fp16.onnx" >&2
    echo "  place it at $PREFIX/query-classifier/model_fp16.onnx" >&2
  }
fi
